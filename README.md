...
1. **A human confirms every submission.** Upwork's MCP gates each write behind a draft-confirm step, and its Terms of Use prohibit tools that submit without a person in the loop. This turns out to be structural rather than a setting: submission requires a separate `confirm_draft` call, editing a draft invalidates the id that was approved, and the per-tool permission setting explicitly cannot bypass the gate. There is no auto-fire mode to decline to build.
